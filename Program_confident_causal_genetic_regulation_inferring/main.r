...
CNN_bulk_model_result_directory = '/media/ggj/My Passport1/Xiaoyanyu/CNN/predict_result/OtherTissue/bulk_model/' # predictions of CNN (original bulk-level model)
...
allcelltype_top_assoc_file = '/home/ggj/QTL/all_top_assoc_allcelltype.txt' # top ieqtl for all cell types
...
file_list <- list.files(path = CNN_bulk_model_result_directory)
file_list

setwd(CNN_bulk_model_result_directory)
bulk <- data.frame()
for (i in file_list){
   bulk_tmp <- fread(i,sep=',',stringsAsFactors = F)
   bulk_tmp <- as.data.frame(bulk_tmp)
   bulk <- rbind(bulk,bulk_tmp)
  }

bulk$variant_ID <- paste0(bulk$'0','_',bulk$'1','_',bulk$'3','_',bulk$'4')
bulk$index <- paste0(bulk$variant_ID,'_',bulk$gene)
bulk <- bulk[!duplicated(bulk$index),]
bulk <- bulk[nchar(as.character(bulk$'3'))%in%1&nchar(as.character(bulk$'4'))%in%1, ]

all_tissues <- colnames(bulk)[grep(colnames(bulk),pattern='Adult')]

result_bulk_all <- data.frame()
for (i in all_tissues){
   bulk_tmp <- data.frame(bulk[,c('variant_ID','gene','dist',i)])
   colnames(bulk_tmp) <- c('variant_ID','gene','dist','bulk')
   bulk_tmp$tissue <- rep(i,nrow(bulk_tmp))
   result_bulk_all <- rbind(result_bulk_all,bulk_tmp)
 }
match1 <- match[match$index%in%as.character(result_bulk_all$variant_ID), ]
match1 <- match1[!duplicated(match1$index), ]
rownames(match1) <- match1$index

result_bulk_all$variant_ID_h38 <- match1[as.character(result_bulk_all$variant_ID),]$V7

write.table(result_bulk_all,file='result_bulk_all.txt',quote=F,row.names=F,col.names=T,sep='\t' )
...
single_alltissue <- fread('/media/ggj/My Passport1/Xiaoyanyu/CNN/predict_result/OtherTissue/single.txt',sep='\t',stringsAsFactors = F)
...
ss <- fread('/media/ggj/My Passport1/Xiaoyanyu/CNN/predict_result/AdultArtery_200kb/single.txt',sep='\t',stringsAsFactors = F)##
ss  <- as.data.frame(ss)
